...
import { HistoricoCupon, HistoricoCuponDocument } from 'src/historico-cupon/schemas/historico-cupon.schema';
...
export class CuponService {
  constructor(
    ...
    // ⬇️ NUEVO
  @InjectModel(Usuario.name) private readonly _usuarioModel: Model<UsuarioDocument>,
  @InjectModel(HistoricoCupon.name) private readonly _historicoModel: Model<HistoricoCuponDocument>,
  ) {}
...
async obtenerCuponerasPorCliente(
  ...
  async asignarCuponACliente(cuponId: string, clienteId: string, { override = false } = {}) {
    if (!Types.ObjectId.isValid(cuponId) || !Types.ObjectId.isValid(clienteId)) {
...
async obtenerDetalleCupon(cuponId: string) {
  ...
  try {
    ...
    const cupon: any = await this._cuponModel
      ...
      .populate({
        path: 'version',
        select: 'nombre estado ciudadesDisponibles numeroDeLocales descripcion',
        populate: { path: 'ciudadesDisponibles', select: 'nombre', options: { lean: true } },
        options: { lean: true },
      })
      .lean()
      .exec();
    ...
    if (!cupon) throw new NotFoundException('Cupón no encontrado');
    if (!cupon.version) throw new NotFoundException('Versión de cuponera no encontrada');

    const version = cupon.version;
    const ciudadesVersionNombres: string[] = Array.isArray(version.ciudadesDisponibles)
      ? version.ciudadesDisponibles.map((c: any) => c?.nombre).filter(Boolean)
      : [];

    const ciudadesVersionIds: Types.ObjectId[] = Array.isArray(version.ciudadesDisponibles)
    ...
    // Paso 2: Candidatos
    const ciudadFilter = ciudadesVersionIds.length > 0 ? { ciudades: { $in: ciudadesVersionIds } } : {};
    ...
    const agg = await this._historicoModel.aggregate<{
      ...
      { $match: { cupon: cuponId } },
      { $group: { _id: '$usuario', count: { $sum: 1 }, lastScan: { $max: '$fechaEscaneo' } } },
    ]);
    ...
    for (const row of agg) {
      ...
      if (!uid) continue;
      const info = candidatosMap.get(uid);
      if (!info) continue;
    ...
    const lugaresSinScannear = candidatos
      ...
      .map((c) => ({
        ...
        nombre: c.nombre,
        email: c.email,
        ciudades: (Array.isArray(c.ciudades) ? c.ciudades : []).map((x: any) => x?.nombre).filter(Boolean),
    ...
    return {
      cupon: {
        ...
        fechaActivacion: cupon.fechaActivacion ? new Date(cupon.fechaActivacion).toISOString() : null,
        fechaVencimiento: cupon.fechaVencimiento ? new Date(cupon.fechaVencimiento).toISOString() : null,
        ultimoScaneo: cupon.ultimoScaneo ? new Date(cupon.ultimoScaneo).toISOString() : null,
      },
      version: {
      ...
      lugaresScaneados: lugaresScaneados.sort(
        (a, b) => (b.lastScan ? Date.parse(b.lastScan) : 0) - (a.lastScan ? Date.parse(a.lastScan) : 0),
      ),
      lugaresSinScannear,
    ...
    this.logger.error(`[obtenerDetalleCupon] ERROR: ${err?.message}`, err?.stack);
    throw err;
  }
}
